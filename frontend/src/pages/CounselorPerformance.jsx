...
import { performanceAPI, collegesAPI, targetsAPI, admissionsAPI } from '../lib/api';
...
export default function CounselorPerformance() {
  ...
  const isAdmissionManager = user?.designation === 'Admission Manager';
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Get initial admissions list
      const admissionsRes = await performanceAPI.getAdmissions({});
  ...
  const fetchFilteredData = async () => {
    try {
      ...
      const [statsRes, admissionsRes] = await Promise.all([
        performanceAPI.getStats(params),
        performanceAPI.getAdmissions(params)
      ]);
  ...
  }, [filterCounselor, filterCollege, filterMonth]);
  ...
  const hasActiveFilters = filterCounselor !== 'all' || filterCollege !== 'all' || filterMonth !== 'all';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-heading font-bold text-[#0F172A]">
              {isAdmissionManager ? 'Team Performance' : 'My Team Performance'}
            </h1>
            <p className="text-[#475569] font-body mt-1">
              {isAdmissionManager 
                ? 'Complete overview of all counselor performance'
                : 'Performance overview of your team members'
              }
            </p>
          </div>
          {canAssignTargets && (
          ...
          <CardContent className="p-4">
            <div className="flex flex-col md:flex-row gap-4 items-end">
              <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <Label className="text-xs text-[#94A3B8] mb-1 block">By Month</Label>
                ...
                <div>
                  <Label className="text-xs text-[#94A3B8] mb-1 block">By College</Label>
                  <Select value={filterCollege} onValueChange={setFilterCollege}>
                    <SelectTrigger data-testid="filter-college">
        ...
        <Tabs defaultValue="targets">
          <TabsList>
            ...
            <TabsTrigger value="trends" className="font-body">
            ...
            <Card>
              ...
              <CardContent>
                {stats?.monthly_trends?.length > 0 ? (
                    ...
                    {stats.monthly_trends.map((item, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-body font-medium">{item._id}</span>
                        </div>
                        <div className="flex items-center gap-6">
                          <div className="text-right">
                            <p className="text-xs text-[#94A3B8]">Admissions</p>
                            <p className="font-heading font-bold">{item.count}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-xs text-[#94A3B8]">Fees Collected</p>
                            <p className="font-heading font-bold text-green-600">{formatCurrency(item.fees_collected)}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <p className="text-center text-[#94A3B8] py-12">No trend data available</p>
                )}
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="font-heading text-lg">
                  Admitted Students ({admissionsList.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription className="font-body">
              Set admission and fee collection targets for team members
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="outline" size="sm" onClick={handleAddInstalment}>
            ...
            <div>
              ...
              <Input value={admissionForm.remark} onChange={e => setAdmissionForm(p => ({ ...p, remark: e.target.value }))} placeholder="Any notes..." className="mt-1" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditAdmissionOpen(false)}>Cancel</Button>
            <Button onClick={handleSaveAdmission} disabled={savingAdmission} className="bg-[#0066CC] hover:bg-[#0052A3]">
